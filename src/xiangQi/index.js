...
function PieceView(props) {
  return (
    ...
    <div
      className={
        props.data.conData
          ? props.data.conData.type * 1 === 0
            ? props.choose
              ? "piece black className"
              : "piece black"
            : props.data.conData.type * 1 === 1
            ? props.choose
              ? "piece red className"
              : "piece red"
            : "piece"
      ...
      style={{ left: props.data.col * 80 + 50, top: props.data.lin * 80 + 50 }}
...
class Xiangqi extends React.Component {
    ...
    if (now.conData.con === "卒" || now.conData.con === "兵") {
      if (
        now.conData.con === "卒"
          ? next.col === now.col
            ? now.lin - next.lin === 1
            : now.lin === next.lin && now.lin < 5
            ? Math.abs(next.col - now.col) === 1
            : false
          : next.col === now.col
          ? next.lin - now.lin === 1
          : now.lin === next.lin && now.lin > 4
          ? Math.abs(next.col - now.col) === 1
          : false
      ) {
        success();
      ...
      if (
        (Math.abs(next.col - now.col) === 2 &&
          Math.abs(next.lin - now.lin) === 1) ||
        (Math.abs(next.col - now.col) === 1 &&
          Math.abs(next.lin - now.lin) === 2)
      ) {
      ...
      if (
        (now.conData.con === "象" ? next.lin < 5 : next.lin > 4) &&
        ((Math.abs(next.col - now.col) === 2 &&
          Math.abs(next.lin - now.lin) === 2) ||
          (Math.abs(next.col - now.col) === 2 &&
            Math.abs(next.lin - now.lin) === 2))
      ...
      if (
        ...
        next.col > 2 &&
        next.col < 6 &&
        (now.conData.con === "士" ? next.lin < 3 : next.lin > 6)
      ) {
        success();
        delete obj_.conData;
        list[next.col][next.lin]["conData"] = now.conData;
      ...
      if (
        ((Math.abs(next.col - now.col) === 0 &&
          Math.abs(next.lin - now.lin) === 1) ||
          (Math.abs(next.col - now.col) === 1 && next.lin - now.lin === 0)) &&
        next.col > 2 &&
        next.col < 6 &&
        (now.conData.con === "将" ? next.lin < 3 : next.lin > 6)
      ) {
        success();
        delete obj_.conData;
        list[next.col][next.lin]["conData"] = now.conData;
        ...
        for (var i = 0; i < list[next.col].length; i++) {
          if (
            list[next.col][i].conData &&
            list[next.col][i].conData.con !== "将" &&
            list[next.col][i].conData.con !== "帅"
          ) {
            break;
          }
        }
      ...
      if (next.col === now.col || next.lin === now.lin) {
        if (next.col === now.col) {
          ...
          let i_max,
            i_min = 0;
          i_max = next.lin > now.lin ? next.lin : now.lin;
          i_min = next.lin > now.lin ? now.lin : next.lin;
      ...
      if (next.col === now.col) {
        ...
        let i_max,
          i_min = 0;
        i_max = next.col > now.col ? next.col : now.col;
        i_min = next.col > now.col ? now.col : next.col;
        for (let i = i_max - 1; i > i_min; i--) {
          if (list[i][now.lin].conData) {
            p_++;
          }
  ...
  changePiece(l, c) {
    ...
    if (this.state.check.length === 0) {
      if (next.conData) {
        if (
          (next.conData.type === 1 && this.state.player) ||
          (next.conData.type === 0 && !this.state.player)
        ) {
          this.setState({
            check: [l, c],
          });
      ...
      if (next.conData) {
        if (
          (now.conData.type === 1 && this.state.player) ||
          (now.conData.type === 0 && !this.state.player)
        ) {
  ...
  render() {
      ...
      return (
        <p
          ...
          className={
            ...
            (index >= 50 && index <= 58)
              ? (index % 10 === 0 && index !== 0) ||
                ((index - 9) % 10 === 0 && index !== 9)
                ? "tables no-top no-right "
                : "tables no-right"
              : (index % 10 === 0 && index !== 0) ||
                ((index - 9) % 10 === 0 && index !== 9)
              ? "tables no-top"
              : index === 14 || index === 25 || index === 84 || index === 95
              ? "f"
              : index === 15 || index === 24 || index === 85 || index === 94
              ? "x"
              : "tables"
          }
        ></p>
        ...
        <div key={index}>
          {item.map((item1, number) => {
            return (
              <PieceView
                choose={
                  this.state.check[0] === number &&
                  this.state.check[1] === index
                    ? true
                    : false
                }
                key={number}
    ...
    let player = this.state.gameOver
      ? "GAME  OVER"
      : this.state.player
      ? "红方回合"
      : "黑方回合";
    let winner = this.state.gameOver
      ? this.state.player === 1
        ? "红方胜利"
        : "黑方胜利"
      : "";
